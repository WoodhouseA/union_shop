...
-   [x] **Create `home_page.dart` file:** This file will contain the `HomePage` widget.
    *   **Reasoning:** To separate the homepage UI from other parts of the app, improving modularity.
-   [x] **Implement `HomePage` as a `StatelessWidget`:**
...
-   [x] **Build the layout using a `Scaffold` widget:** The `Scaffold` will have an `appBar` and a `body`.
    *   **Reasoning:** `Scaffold` provides a standard visual layout structure for Material Design apps.
-   [x] **Add a `Column` or `ListView` to the `body` for vertical arrangement of content.**
    *   **Reasoning:** To display promotional banners, featured products, and other sections vertically.
-   [x] **Create static widgets for different sections:**
...
-   [x] **Create `about_us_page.dart` file:** This will contain the `AboutUsPage` widget.
-   [x] **Implement `AboutUsPage` as a `StatelessWidget`:**
    *   **Reasoning:** The content is static and informational.
-   [x] **Use a `Scaffold` with an `appBar` titled "About Us".**
-   [x] **Add a `SingleChildScrollView` to the `body` to ensure content is scrollable.**
-   [x] **Display company information using `Text` widgets for paragraphs and headings.**
    *   **Data:** Hardcode the text for the company's history, mission, and team.

### 3. Footer (4%)

-   [x] **Create a `footer.dart` file for a reusable `Footer` widget.**
-   [x] **Implement `Footer` as a `StatelessWidget`:**
    *   **Reasoning:** The footer content is static.
-   [x] **Use a `Container` with a distinct background color.**
-   [x] **Use a `Row` or `Column` to layout dummy links and information.**
...
-   [x] **Add the `Footer` widget to the `bottomNavigationBar` or at the bottom of the main content `Column`/`ListView` in at least one page (e.g., `HomePage`).**

### 4. Dummy Collections Page (5%)

-   [x] **Create `collections_page.dart` for the `CollectionsPage` widget.**
-   [x] **Implement `CollectionsPage` as a `StatelessWidget`:**
    *   **Reasoning:** Displaying a static list of collections.
-   [x] **Use a `Scaffold` with an `appBar` titled "Collections".**
-   [x] **Use a `GridView` or `ListView` to display collection cards.**
-   [x] **Create a `CollectionCard` widget:**
...
-   [x] **Create `collection_page.dart` for the `CollectionPage` widget.**
-   [x] **Implement `CollectionPage` as a `StatelessWidget`:**
    *   **Reasoning:** The product data within the collection is hardcoded.
-   [x] **Use a `Scaffold` with an `appBar` showing the collection name.**
-   [x] **Add non-functional filter and sort widgets:**
...
-   [x] **Display products using a `GridView` of `ProductCard` widgets with hardcoded data.**

### 6. Dummy Product Page (4%)

-   [x] **Create `product_page.dart` for the `ProductPage` widget.**
-   [x] **Implement `ProductPage` as a `StatelessWidget`:**
    *   **Reasoning:** All product details are hardcoded.
-   [x] **Use a `Scaffold` for the page layout.**
-   [x] **Display product images using a `PageView` or a `Column` of `Image.asset` widgets.**
-   [x] **Show product details:**
    *   **Name:** `Text` widget with a large font size.
    *   **Price:** `Text` widget.
    *   **Description:** `Text` widget.
-   [x] **Add non-functional UI widgets:**
...
-   [x] **Create `sale_collection_page.dart` for the `SaleCollectionPage` widget.**
-   [x] **Implement `SaleCollectionPage` as a `StatelessWidget`:**
    *   **Reasoning:** Displaying a static list of sale items.
-   [x] **Use a `Scaffold` with an `appBar` titled "Sale".**
-   [x] **Display sale products using a `GridView` of `ProductCard` widgets.**
-   [x] **Modify the `ProductCard` to show discounted prices:**
...
-   [x] **Add promotional messaging using `Container` and `Text` widgets.**

### 8. Authentication UI (3%)

-   [x] **Create `auth_page.dart` for the `AuthPage` widget.**
-   [x] **Implement `AuthPage` as a `StatefulWidget` to toggle between Login and Signup forms.**
    *   **State Variable:** `bool _showLoginPage = true;`
-   [x] **Create `login_form.dart` and `signup_form.dart` for the respective form widgets.**
-   [x] **Implement forms using the `Form` widget and `TextFormField` for email and password fields.**
    *   **Reasoning:** `Form` provides validation capabilities.
-   [x] **Add `ElevatedButton` for form submission. The `onPressed` callbacks can be empty.**
-   [x] **Add a `TextButton` to switch between the login and signup views.**

### 9. Static Navbar (5%)

-   [x] **Create `navbar.dart` for a reusable `NavBar` widget.**
-   [x] **Implement `NavBar` as a `StatelessWidget`:**
-   [x] **Use a `LayoutBuilder` to create a responsive navigation bar.**
...
-   [x] **Integrate the `NavBar` into the `appBar` of the `Scaffold` on the main pages.**
...
-   [x] **Create a `CollectionService` (`collection_service.dart`):**
...
-   [x] **Convert `CollectionsPage` to a `StatefulWidget`:**
    *   **Reasoning:** To manage the state of the fetched collections, sorting, and filtering.
-   [x] **Fetch data in `initState` using the `CollectionService`.**
    *   **State Variable:** `late Future<List<Collection>> _collectionsFuture;`
-   [x] **Use a `FutureBuilder` to display a loading indicator and then the `GridView` of collections.**
...
-   [x] **Create a `Product` data model (`product_model.dart`):**
    *   **Class:** `class Product { final String id; final String name; final double price; final String imageUrl; ... }`
-   [x] **Create a `ProductService` (`product_service.dart`):**
    *   **Method:** `Future<List<Product>> getProductsByCollection(String collectionId)`
-   [x] **Convert `CollectionPage` to a `StatefulWidget`:**
-   [x] **Fetch products in `initState` based on the `collectionId` passed to the widget.**
-   [x] **Use a `FutureBuilder` to handle the async data fetching.**
-   [x] **Implement functional sorting, filtering, and pagination widgets:**
...
-   [x] **Convert `ProductPage` to a `StatefulWidget`:**
    *   **Reasoning:** To manage the state of selected options (size, color) and quantity.
-   [x] **Fetch product data using `ProductService` based on a `productId`.**
-   [x] **Populate the UI with data from the `Product` object.**
-   [x] **Implement functional dropdowns and counters:**
...
-   [x] **Create a `CartItem` model (`cart_model.dart`):**
    *   **Class:** `class CartItem { final Product product; int quantity; }`
-   [x] **Create a `CartService` (`cart_service.dart`) using a state management solution (e.g., `Provider`, `Riverpod`):**
...
-   [x] **Implement "Add to Cart" functionality on the `ProductPage`.**
-   [x] **Create `cart_page.dart`:**
    *   **UI:** Display a `ListView` of `CartItem` widgets, showing product details, quantity, and a total price.
-   [x] **Implement a checkout button that simulates placing an order (e.g., shows a confirmation dialog and clears the cart).**

### 5. Print Shack (3%)

-   [x] **Create `print_shack_page.dart` and `print_shack_about_page.dart`.**
-   [x] **Implement the `PrintShackPage` as a `StatefulWidget`:**
    *   **Reasoning:** The form fields will dynamically update the UI.
-   [x] **Create a form for text personalization:**
...
-   [x] **Create a preview widget that updates in real-time as the user types or selects options.**
    *   **Logic:** Use the `onChanged` callbacks of the form fields to call `setState` and rebuild the preview widget with the new styles.

### 6. Navigation (3%)

-   [x] **Set up a routing solution (e.g., `Navigator 2.0` with `Router`, or a package like `go_router`).**
    *   **Reasoning:** To handle named routes and URL-based navigation.
-   [x] **Define routes for all pages (e.g., `/`, `/collections`, `/product/:id`).**
-   [x] **Update all navigation logic (`onPressed` callbacks for buttons, links) to use the router for navigation.**
    *   **Example:** `context.go('/product/123')` with `go_router`.

### 7. Responsiveness (5%)

-   [x] **Review all pages and ensure they are adaptive.**
-   [x] **Use `LayoutBuilder`, `MediaQuery`, and responsive widgets like `Wrap`, `FittedBox`, and `AspectRatio`.**
-   [x] **Test the layout on different screen sizes by resizing the window in desktop mode.**
